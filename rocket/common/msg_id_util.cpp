...
namespace rocket
{
    ...
    std::string MsgIdUtil::GenMsgID()
    {
        if (t_msg_id_num.empty() || t_msg_id_num == t_max_msg_id_num) // 还没有生成msg_id
        {
            if (g_random_fd == -1)
            {
                g_random_fd = open("/dev/urandom", O_RDONLY); // 获取随机文件的句柄
            }
            std::string res(g_msg_id_length, 0);
            if (read(g_random_fd, &res[0], g_msg_id_length) != g_msg_id_length) // 获取文件中的随机字符串
            {
        ...
        else
        {
            ...
            {
                t_msg_id_num[i] += 1;
                for (size_t j = i + 1; i < t_msg_id_num.length(); i++)
                {
                    t_msg_id_num = '0';
                }
            }
